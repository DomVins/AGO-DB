...
class SubjectAssessments extends StatefulWidget {
  ...
  @override
  State<SubjectAssessments> createState() => _SubjectAssessmentsState();
}

class _SubjectAssessmentsState extends State<SubjectAssessments> {
  Box<Class> classBox;
...
class _SpecialTableState extends State<SpecialTable> {
  ...
  void initState() {
    ...
    for (int i = 0; i < subjectTakers[0].subjects.length; i++) {
      if (subjectTakers[0].subjects[i].name ==
          MainStore.classes[widget.classIndex].terms[widget.termIndex]
              .subjects[widget.subjectIndex]) {
        innerIndex = i;
        break;
  ...
  _textField(int id, int stdIndex) {
    return TableCell(
      ...
      child: Center(
        ...
        initialValue: _textFormValue(id, stdIndex),
        onChanged: (value) {
          if (value.isNotEmpty) {
            switch (id) {
              case 1:
                subjectTakers[stdIndex]
